...
import { downloadPlaybookPDF } from '../utils/pdfGenerator';
import { 
  BookOpen, 
  Download, 
  CheckCircle2, 
...
export const PlaybookSeriesCenter: React.FC<PlaybookSeriesCenterProps> = ({ onOpenProduct }) => {
  ...
  const handleDownload = () => {
    ...
    try {
      downloadPlaybookPDF(activePlaybook);
    } catch (err) {
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#1C1917]/30 text-xs">•</span>
                <span className="text-xs text-[#1C1917]/60">Digital-First PDF Resale Specs</span>
              </div>
              ...
              <p className="text-[#1C1917]/75 font-serif italic text-sm sm:text-base max-w-3xl">
                Publication-ready, vector digital ebooks engineered without crop marks, bleed, or print specs. Complete with embedded fonts, searchable text, clickable Table of Contents, verified sourcing matrices, and mandatory Personal Use Licenses.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="bg-[#1C1917] hover:bg-[#4A533E] text-[#FAF8F5] text-xs font-semibold px-4 py-3 border border-[#1C1917] rounded-xs shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
              >
                <Download className="w-4 h-4" />
                <span>{isDownloading ? 'Generating PDFs...' : 'Download All 11 Playbooks (ZIP/Bundle)'}</span>
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    disabled={isDownloading}
                    className="bg-[#4A533E] hover:bg-[#3B4232] text-[#FAF8F5] text-xs font-semibold px-5 py-2.5 rounded-xs shadow-xs transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    <Download className="w-4 h-4" />
                    <span>{isDownloading ? 'Building Vector PDF...' : 'Download Commercial PDF'}</span>
                  </button>
                </div>
